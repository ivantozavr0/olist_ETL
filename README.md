...
Данный проект реализует end-to-end платформу обработки данных
для аналитики e-commerce.

Пайплайн автоматически загружает данные, обрабатывает их
в Spark кластере, формирует аналитическое хранилище
на ClickHouse и предоставляет BI дашборды в Metabase.

## Ключевые особенности проекта

- Реализован end-to-end пайплайн обработки данных
...
- В проекте используется MinIO как S3-совместимое хранилище
для реализации Data Lake (Bronze и Silver слои).
...
- avg_delivery_days: среднее время доставки товаров данного продавца в днях
...
- late_delivery_rate: процент не доставленных вовремя заказов в данный город
...
- Добавление инкрементальной загрузки данных
- Добавление data quality checks
- Интеграция со streaming (Kafka)
